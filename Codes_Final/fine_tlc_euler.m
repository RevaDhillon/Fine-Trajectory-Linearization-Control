...
[T_final, w_final, T_tilda, q_final, q_bar, w_tilda_star, w_err, yaw_d, yaw_found1, yaw_found2, pitch_d, pitch_found1, pitch_found2, roll_d, roll_found1, roll_found2] = fine_TLC(t_ode, tau, c);
[T_final_stan,w_final_stan, T_tilda_stan, q_final_stan, q_bar_stan, w_tilda_star_stan, w_err_stan, yaw_d_stan, yaw_found1_stan, yaw_found2_stan, pitch_d_stan, pitch_found1_stan, pitch_found2_stan, roll_d_stan, roll_found1_stan, roll_found2_stan] = standard_TLC(t_ode, tau, c);

%% Plotting Block:
% 
% %Roll:
% figure(10)
% % plot(t_ode, yaw_found2*180/pi, 'LineWidth', 3, 'LineStyle','-', 'Color', 'b', 'DisplayName','Fine TLC-1')
% % hold on
% % plot(t_ode, yaw_found2_stan*180/pi, 'LineWidth', 1.5, 'LineStyle','-', 'Color', 'r', 'DisplayName','Traditional TLC-1')
% plot(t_ode, roll_found1*180/pi, 'LineWidth', 1.5, 'LineStyle','-', 'Color', 'b', 'DisplayName','Fine TLC-2')
% hold on
% plot(t_ode, roll_found1_stan*180/pi, 'LineWidth', 1.5, 'LineStyle','-', 'Color', 'r', 'DisplayName','Traditional TLC-2')
% 
...
% %Omega:
% figure(4)
% plot(t_ode, (180/pi)*sqrt(w_final(1, :).^2 + w_final(2, :).^2 + w_final(3, :).^2), 'LineWidth', 3, 'LineStyle','-', 'Color', 'b', 'DisplayName','Fine TLC')
% hold on
% plot(t_ode, (180/pi)*sqrt(w_final_stan(1, :).^2 + w_final_stan(2, :).^2 + w_final_stan(3, :).^2), 'LineWidth', 1.5, 'LineStyle','-', 'Color', 'r', 'DisplayName','Traditional TLC')
% legend("show")
% ax = gca;
% ax.FontSize = 16;
% ylim(ax, [0, 4])
% grid on
% title('Angular Velocity Magnitude Variation')
% xlabel("Time (s) -->")
% ylabel("\omega (deg/s) -->")

figure(4)
plot(t_ode, sqrt(T_final(1, :).^2 + T_final(2, :).^2 + T_final(3, :).^2), 'LineWidth', 3, 'LineStyle','-', 'Color', 'b', 'DisplayName','Fine TLC')
hold on
plot(t_ode, sqrt(T_final_stan(1, :).^2 + T_final_stan(2, :).^2 + T_final_stan(3, :).^2), 'LineWidth', 1.5, 'LineStyle','-', 'Color', 'r', 'DisplayName','Traditional TLC')
legend("show")
ax = gca;
ax.FontSize = 16;
ylim(ax, [0, 0.35])
grid on
title('Control Torque Magnitude Variation')
xlabel("Time (s) -->")
ylabel("Magnitude of T (Nm) -->")
...
function [T_final, w_final, T_tilda, q_final, q_bar, w_tilda_star, w_err, yaw_d, yaw_found1, yaw_found2, pitch_d, pitch_found1, pitch_found2, roll_d, roll_found1, roll_found2] = fine_TLC(t_ode, tau, c)
...
function [T_final, w_final, T_tilda, q_final, q_bar, w_tilda_star, w_err, yaw_d, yaw_found1, yaw_found2, pitch_d, pitch_found1, pitch_found2, roll_d, roll_found1, roll_found2] = standard_TLC(t_ode, tau, c)
%%% Spacecraft model parameters (Wu et al., 2018):
